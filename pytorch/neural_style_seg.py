...
style_fg = image_loader("images/lily.jpg").type(dtype)
style_bg = image_loader("images/picasso.jpg").type(dtype)
content_img = image_loader("images/me1.jpeg").type(dtype)
mask_img = cv2.imread("images/mask.png")
# mask_img = np.ones([3, 244,244])*100
mask_img = np.array(mask_img<50, dtype=bool)
...
def imshow(tensor, title=None):
    ...
    imsz = image.shape
    image = image.view(3, imsz[-2], imsz[-1])  # remove the fake batch dimension
...
class StyleLoss(nn.Module):
    ...
    def __init__(self, target_fg, target_bg, weight):
        super(StyleLoss, self).__init__()
        self.target_fg = target_fg.detach() * weight
        self.target_bg = target_bg.detach() * weight
        self.weight = weight
    ...
    def forward(self, input):
        ...
        ## edit for heterogeneous algorithm
        mask_input = cv2.resize(mask_img[:, :, 0].astype(np.float32), (input.shape[-1], input.shape[-2]))
        mask_input = np.repeat(mask_input[np.newaxis, :, :], input.shape[-3], axis=0)
        mask_input = mask_input[np.newaxis,:]
        self.fg = input * Variable(torch.FloatTensor(abs(1-mask_input)))
        self.bg = input * Variable(torch.FloatTensor(mask_input))

        self.G_fg = self.gram(self.fg)
        self.G_fg.mul_(self.weight)

        self.G_bg = self.gram(self.bg)
        self.G_bg.mul_(self.weight)
        self.loss = self.criterion(self.G_fg, self.target_fg)
        return self.output
...
def get_style_model_and_losses(cnn, style_fg, style_bg, content_img,
    ...
    for layer in list(cnn):
        if isinstance(layer, nn.Conv2d):
            ...
            feature_model.add_module(name, layer)

            if name in content_layers:
                # add content loss:
                target = feature_model(content_img).clone()
            ...
            if name in style_layers:
                # add style loss:
                target_feature_fg = feature_model(style_fg).clone()
                target_feature_bg = feature_model(style_bg).clone()
                mask_fg = cv2.resize(mask_img[:, :, 0].astype(np.float32), (target_feature_fg.shape[-1], target_feature_fg.shape[-2]))
                mask_fg = np.repeat(mask_fg[np.newaxis, :, :], target_feature_fg.shape[-3], axis=0)
                mask_fg = mask_fg[np.newaxis, :]
                target_feature_fg = target_feature_fg*Variable(torch.FloatTensor(abs(1-mask_fg)))

                mask_bg = cv2.resize(mask_img[:, :, 0].astype(np.float32),
                                     (target_feature_bg.shape[-1], target_feature_bg.shape[-2]))
                mask_bg = np.repeat(mask_bg[np.newaxis, :, :], target_feature_bg.shape[-3], axis=0)
                mask_bg = mask_bg[np.newaxis, :]

                target_feature_bg = target_feature_bg*Variable(torch.FloatTensor(mask_bg))
                #heterogeneous
                target_feature_gram_fg = gram(target_feature_fg)
                target_feature_gram_bg = gram(target_feature_bg)
                style_loss = StyleLoss(target_feature_gram_fg, target_feature_gram_bg, style_weight)
        ...
        if isinstance(layer, nn.ReLU):
            ...
            if name in style_layers:
                # add style loss:
                target_feature = feature_model(style_img).clone()
...
def get_input_param_optimizer(input_img):
    ...
    optimizer = optim.Adam([input_param], lr=0.1)
...
def run_style_transfer(cnn, content_img, style_fg, style_bg, input_img, num_steps=300,
                       style_weight=10000, content_weight=1):
    """Run the style transfer."""
    print('Building the style transfer model..')
    model, style_losses, content_losses = get_style_model_and_losses(cnn,
        style_fg, style_bg, content_img, style_weight, content_weight)
    ...
    while run[0] <= num_steps:
        ...
        def closure():
            ...
            grad_test = input_param.grad
            imshow(grad_test.data)
            ...
            if run[0] % 50 == 0:
                ...
                print()
            closure_loss = style_score + content_score
            print(closure_loss)
            return closure_loss
...
output = run_style_transfer(cnn, content_img, style_fg, style_bg, input_img)
